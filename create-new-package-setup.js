...
let correctPassword = "00luxpath00";
...
/* Auto-format price input with thousands commas as the user types */
function formatPriceInput(input) {
    let raw = input.value.replace(/[^0-9]/g, '');
    input.value = raw === '' ? '' : Number(raw).toLocaleString('en-US');
}

/* Function to show or hide the total price page in the downloaded PDF */
function togglePriceDisplayInPDF() {
    ...
    let pricePage = document.getElementById('downloaded_pdf_total_price_data_page');
    let priceEl = document.getElementById('package_total_price_pdf_p_id');

    if (showPackageTotalPriceCheckbox.checked && priceEl.innerText.trim() !== '') {
...
websiteUsersNameInputOptions.forEach(option => {
    option.addEventListener('click', () => {
        ...
        let newValue = `بكج ${option.textContent}`;
