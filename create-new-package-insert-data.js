    ...
    if (wholePackageStartDateInput === '' || wholePackageEndDateInput === '' || websiteUsersNameInput === '') {
        ...
        if (document.getElementById('website_users_name_input_id').disabled !== true) {
            ...
            if (document.getElementById('package_user_code_name_for_later_import_reference_p_id').innerText === '' || document.getElementById('website_users_name_input_id').value !== lastCLickedWebsiteUserNameVariable) {
                ...
                if (document.getElementById('website_users_name_input_id').value === 'بكج بندر') {
                    websiteUsersNameInput = 'br';

                } else if (document.getElementById('website_users_name_input_id').value === 'بكج احمد') {
                    websiteUsersNameInput = 'ad';

                } else if (document.getElementById('website_users_name_input_id').value === 'بكج رايد') {
                    websiteUsersNameInput = 'rd';
        ...
        document.getElementById('package_clint_code_number_p_id').setAttribute('data-has-value', packageClintCodeNumberInput.trim() ? 'true' : 'false');
...
createAllPackageIncludingAndNotIncludingData = function () {
    ...
    // Read the SAR total from the price table (e.g. "8,043 SAR") and extract just the number ("8,043")
    let _packageTotalPriceRaw = (document.getElementById('package_total_price_p_id')?.innerText || '').trim();
    let packageTotlaPriceInput = (_packageTotalPriceRaw.match(/[\d,]+/) || [''])[0];
    ...
    if (packageTotlaPriceInput !== '' && packageTotlaPriceInput !== '0') {
        ...
        document.getElementById('package_total_price_pdf_p_id').innerText = packageTotlaPriceInput;
        ...
        document.getElementById('package_total_price_pdf_p_id').innerText = '';
    ...
    checkboxIds.forEach(id => {
        ...
        if (color === 'rgb(0, 255, 0)') {
            ...
            const _includeIconMap = {
                'privet_car_with_driver_to_welcome_and_etc_checkbox': 'car-sport',
                'hotel_booking_with_breakfast_for_2_people_checkbox': 'bed',
                'customer_service_24_hour_checkbox': 'headset',
                'welcome_goodbye_hotel_delivery_checkbox': 'people',
                'inner_flight_tickets_checkbox': 'airplane',
                'extra_car_for_carring_bags_checkbox': 'bag-handle',
                'outer_flight_tickets_checkbox': 'paper-plane',
                'placese_visiting_cost_checkbox': 'camera',
                'bali_taxes_not_covered_checkbox': 'card',
            };
            icon.setAttribute('name', _includeIconMap[id] || 'checkmark-outline');
            p.appendChild(icon);
            if (id === 'sms_card_with_internet_checkbox') {
                let textContent = smsCardWithInternetAmountInputReayText !== ''
                    ? ` ${smsCardWithInternetAmountInputReayText}`
    ...
    // Equalize all including card heights to the tallest card
    setTimeout(() => {
        let _cards = insertedPackageIncludingDataDiv.querySelectorAll('p');
        let _maxH = 0;
        _cards.forEach(function (p) { p.style.height = 'auto'; _maxH = Math.max(_maxH, p.offsetHeight); });
        _cards.forEach(function (p) { p.style.height = _maxH + 'px'; });
    }, 0);
    ...
    // Populate page title with package type
    let _pkgTypeEl = document.getElementById('clint_package_type_h6');
    let _pkgTitleEl = document.getElementById('pdf_including_page_package_type_p');
    if (_pkgTitleEl && _pkgTypeEl) _pkgTitleEl.innerText = _pkgTypeEl.innerText.trim();
...
runDeleteThisPackageIncludingDataText = function (clickedPackageIncludingDataText) {
    ...
    passClickedPackageIncludingDataText = function () {
        ...
        if (clickedPackageIncludingDataText === 'package_total_price_pdf_p_id') {
            document.getElementById('downloaded_pdf_total_price_data_page').style.display = 'none';
...
downloadPdfWithCustomName = async function (pdfName) {
    ...
    let captureCanvas = async function (section, scale) {
        ...
        try {
            ...
            // Pre-render specific Arabic title/badge elements to canvas images.
            // html2canvas computes x positions from LTR layout, so forcing direction
            // on the context alone places glyphs at the wrong offset. Instead we
            // draw each element ourselves with ctx.direction='rtl', swap it to an
            // <img>, let html2canvas capture the image, then restore the originals.
            var _re = /[؀-ۿ]/;
            section.querySelectorAll(
                '#pdf_page1_welcome_title_p, #clint_package_type_h6, .pdf_section_title_div_class h6'
            ).forEach(function (el) {
                var text = el.textContent.trim();
                if (!text || !_re.test(text)) return;
                var rect = el.getBoundingClientRect();
                var totalW = Math.ceil(rect.width), totalH = Math.ceil(rect.height);
                if (totalW < 2 || totalH < 2) return;
                var cs = window.getComputedStyle(el);
                // Use content-box dimensions so the replacement img doesn't inflate
                // the element beyond its original rendered size (padding + border stay
                // as the element's own CSS, only the content area gets the img).
                ...
                var borH = (parseFloat(cs.borderLeftWidth) || 0) + (parseFloat(cs.borderRightWidth) || 0);
                var borV = (parseFloat(cs.borderTopWidth) || 0) + (parseFloat(cs.borderBottomWidth) || 0);
                var w = Math.max(1, totalW - padH - borH);
                var h = Math.max(1, totalH - padV - borV);
                ...
                _saved.push({ el: el, html: el.innerHTML });
                el.innerHTML = '';
                el.appendChild(img);
            });
            ...
            _saved.forEach(function (s) { s.el.innerHTML = s.html; });
        }
    };
    ...
    document.getElementById('hotel_links_guide_note_p_id').style.display = 'block';
    ...
    // Always keep important notes, clint movements and total price as separate pages.
    // Render order is enforced below: regular sections → clint movements → total price → important notes.
    ...
    let totalPriceSection = visibleSections.find(s => s.id === 'downloaded_pdf_total_price_data_page');
    let regularSections = visibleSections.filter(s =>
        ...
        s.id !== 'downloaded_pdf_total_price_data_page'
    );
    ...
    // Add Total Price as a separate page AFTER clint movements (if visible)
    if (totalPriceSection) {
        let totalPriceCanvas = await captureCanvas(totalPriceSection, scale);
        if (totalPriceCanvas) {
            let pdfHeight = (totalPriceCanvas.height * pdfWidth) / totalPriceCanvas.width;
            if (!pdf) {
                pdf = new jsPDF('p', 'mm', [pdfWidth, pdfHeight]);
            } else {
                pdf.addPage([pdfWidth, pdfHeight]);
            }
            let imgData = totalPriceCanvas.toDataURL('image/jpeg', 0.9);
            pdf.addImage(imgData, 'JPEG', 0, 0, pdfWidth, pdfHeight, '', 'FAST');
        }
    }
...
runDownloadOnlyClintMovementsDataFunction = async function () {
    ...
    document.getElementById('downloaded_pdf_clint_movements_data_page').querySelector('img').src = '';
    document.getElementById('downloaded_pdf_clint_movements_data_page').querySelectorAll('img')[1].src = '';
};