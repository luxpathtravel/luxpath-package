            ...
            <div class="ppt_total_row">
                ...
                <div class="ppt_total_item">
                    <div class="ppt_total_line">
                        ...
                        <p id="package_total_price_p_id" class="ppt_sar_value" contenteditable="true"
                           title="اضغط للتعديل — أي زيادة عن الحد الأدنى تُحتسب ربحاً"
                           onfocus="pptSarFocus(this)"
                           oninput="pptSarTotalEdited(this)"
                           onblur="pptSarTotalCommit(this)"
...
function _recalcPriceTotal() {
    ...
    if (sarDisplay && document.activeElement !== sarDisplay) {
        sarDisplay.textContent = sarTotal > 0 ? `${Number(sarTotal).toLocaleString('en-US')} SAR` : '-';
    }
}

// SAR base (the minimum) = IDR total ÷ exchange rate, before any profit is added
function _pptGetSarBase() {
    let total = 0;
    document.querySelectorAll('#ppt_tbody .ppt_price_value').forEach(el => {
        total += parseInt(el.dataset.raw) || 0;
    });
    const exchangeRate = window._pptSettings?.exchangeRate || 4700;
    return total > 0 ? Math.round(total / exchangeRate) : 0;
}

// User starts editing the SAR total — strip the "SAR" suffix and select all so it's easy to replace
function pptSarFocus(el) {
...
// While editing — derive the profit from the entered SAR total (extra above the minimum).
// Updates ppt_profit_input_id silently (no recalc, so the field the user is typing in isn't clobbered).
function pptSarTotalEdited(el) {
    const enteredSar = parseInt((el.textContent || '').replace(/[^\d]/g, '')) || 0;
    const sarBase    = _pptGetSarBase();
    let profit = enteredSar - sarBase;
    if (profit < 0) profit = 0; // can't go below the minimum
    window._pptProfitSar = profit;
    const profitInput = document.getElementById('ppt_profit_input_id');
    if (profitInput) profitInput.value = profit > 0 ? Number(profit).toLocaleString('en-US') : '';
}

// Done editing — normalize: re-append "SAR" and snap to the minimum if the user went below it
function pptSarTotalCommit(el) {
    _recalcPriceTotal();
}
