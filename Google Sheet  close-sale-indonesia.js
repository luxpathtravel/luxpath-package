...
const fetchAllCloseSellDataFunction = async () => {
    ...
    hotelElements.forEach((hotelElement) => {
        ...
        // Remove the event listener if it's already attached
        ...
        hotelElements.forEach((hotelElement) => {
            ...
            if (matchingData) {
                ...
                if (allMatchingDates.length > 0) {
                    ...
                    // Store the event handler so we can reference it later
                    const clickHandler = () => {
                        // Get package start and end date input values
                        ...
                        // Convert Arabic date strings to JavaScript Date objects
                        ...
                        // Map to group all availability dates by room type
                        const roomAvailabilityMap = new Map(); // 🧠 Key: roomType, Value: combined availability array

                        // Fill the map: merge availability for the same room type
                        matchingData.forEach(({ roomType, availability }) => {
                        ...
                        const messageParts = []; // Array to store final lines for the modal

                        // Loop through each roomType and its combined date list
                        roomAvailabilityMap.forEach((dates, roomType) => {
                            // Remove duplicate dates by converting to timestamp and back to Date
                            const uniqueDates = [...new Set(dates.map(date => date.getTime()))]
                                .map(ts => new Date(ts))
                                .sort((a, b) => a - b); // Sort dates ascending

                            // Filter only dates within the package date range
                            ...
                            if (filteredDates.length > 0) {
                                // Format date ranges into readable strings
                                const formattedRange = formatDateRanges(filteredDates);

                                // Push the final formatted message to the list
                    ...
                    // Attach the event listener with the handler
                    hotelElement.clickHandler = clickHandler; // Store reference for later removal
